 ...
 const DEFAULT_COLORS = [
   { name: 'Blue', excelBg: '4472C4', excelText: 'FFFFFF' },
   { name: 'Green', excelBg: '70AD47', excelText: 'FFFFFF' },
   { name: 'Purple', excelBg: '7030A0', excelText: 'FFFFFF' },
   { name: 'Orange', excelBg: 'ED7D31', excelText: 'FFFFFF' },
   { name: 'Red', excelBg: 'C00000', excelText: 'FFFFFF' },
   { name: 'Teal', excelBg: '00B0F0', excelText: 'FFFFFF' },
   { name: 'Pink', excelBg: 'FF66CC', excelText: 'FFFFFF' },
   { name: 'Amber', excelBg: 'FFC000', excelText: '000000' },
   { name: 'Gray', excelBg: '808080', excelText: 'FFFFFF' },
   { name: 'Navy', excelBg: '1E3A5F', excelText: 'FFFFFF' },
 ];
 ...
 export function PeriodDialog({
   ...
   useEffect(() => {
     if (open) {
       if (period) {
         ...
         const foundColor = DEFAULT_COLORS.find(c => c.excelBg === period.color.excelBg);
         setSelectedColor(foundColor || DEFAULT_COLORS[0]);
   ...
   const handleSave = () => {
     ...
     onSave({
       ...
       label,
       color: {
         bg: `bg-[#${selectedColor.excelBg}]/20`,
         text: `text-[#${selectedColor.excelBg}]`,
         excelBg: selectedColor.excelBg,
         excelText: selectedColor.excelText,
       },
       billingLogic,
     });
   };
               ...
               {DEFAULT_COLORS.map((color) => (
                 <button
                   ...
                   className={`w-8 h-8 rounded border-2 transition-all ${
                     selectedColor.excelBg === color.excelBg
                       ? 'border-foreground scale-110'
                       : 'border-transparent hover:scale-105'
                   }`}
                   style={{ backgroundColor: `#${color.excelBg}` }}
